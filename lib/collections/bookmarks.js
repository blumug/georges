...
Meteor.methods({
  'bookmarkInsert': function(bookmark) {
    ...
    UpdateTagCollections(bookmark.tags);
    ...
    _.each(bookmark.groups, function(group) {
      Meteor.call("createAddBookmark", Meteor.userId(), Groups.findOne({
        name: group.name
      }));
    });
  ...
  'bookmarkUpdate': function(bookmark, id) {
    ...
    if (oldBookmark) {
      EmptyTagsCollections(oldBookmark.tags, GetIdAll(oldBookmark))
      UpdateTagCollections(bookmark.tags);
      _.each(bookmark.groups, function(group) {
        Meteor.call("createAddBookmark", bookmark.userId, Groups.findOne({
          name: group.name
        }));
      });
      Bookmarks.update({
  ...
  'removeBookmark': function(id) {
    check(id, String);
    EmptyTagsCollections(Bookmarks.findOne(id).tags, GetIdAll(Bookmarks.findOne(id)));
    Bookmarks.remove(id);
    return {
      success: true,
      url: id,
...
GetIdAll = function(bookmark) {
  var tabOfId = [];

  tabOfId.push(Meteor.userId());
  _.each(bookmark.groups, function(group) {
      tabOfId.push(group.creator);
    _.each(group, function(member) {
      tabOfId.push(member.id);
    });
  });
  return tabOfId;
}
