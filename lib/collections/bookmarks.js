...
Meteor.methods({
  ...
  'bookmarkUpdate': function(bookmark, id) {
    ...
    if (!id && Bookmarks.find({
      ...
      Meteor.call("createAddBookmark", Meteor.userId(), bookmark.group);
      _.each(bookmark.groups, function(group) {
      ...
      CreatedTags(bookmark.tags);
       _.each(bookmark.groups, function(group) {
      Meteor.call("createAddBookmark", bookmark.userId, Groups.findOne({name: group.name}));
    });
      Bookmarks.update({
        _id: id
      }, {
  ...
  'removeBookmarkByUrl': function(url, id) {
    check(url, String);
    EmptyTags(Bookmarks.findOne(id));
    Bookmarks.remove({
      userId: id,
      url: url
    });
